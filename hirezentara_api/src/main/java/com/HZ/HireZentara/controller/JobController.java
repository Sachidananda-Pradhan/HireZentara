...
public class JobController extends BaseController {
    ...
    public APIResponse getCandidateListByJobId(@RequestParam String jobId,
                                               @RequestParam Optional<Integer> pageNumber,
                                               @RequestParam Optional<Integer> pageSize,
                                               @RequestParam(defaultValue = "false") boolean isRecent,
                                               @RequestParam Optional<Integer> days,
    ...
    public APIResponse deleteTheJob(@RequestParam String jobId, HttpServletRequest httpRequest) {
        validateSessionId(httpRequest);
    ...
    public ResponseEntity<byte[]> getAllAppliedCandiate(@RequestParam String jobId, HttpServletRequest httpServletRequest) {
        try {
            ...
            byte[] xlsData = jobDetailsService.generateAppliedCandidatesxls(jobId); // Pass jobId if needed
            return buildHttpResponse(xlsData);
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
        }
    }
    private ResponseEntity<byte[]> buildHttpResponse(byte[] xlsData) {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_OCTET_STREAM);
        headers.setContentDisposition(ContentDisposition.builder("attachment")
                .filename("applied_candidates.xls")
                .build());
